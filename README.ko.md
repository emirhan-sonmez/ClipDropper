...
[Windows](#windows) · [iPhone에 설치](#ios) · [소스 코드로 빌드](#소스-코드로-빌드) · [작동 방식](#작동-방식)
...
1. [Releases](https://github.com/emirhan-sonmez/ClipDropper/releases) 페이지에서 `ClipDropper-Setup.exe` 다운로드
2. 설치 프로그램 실행 — .NET 8 데스크톱 런타임이 없으면 자동으로 감지하여 설치
3. 시작 메뉴 또는 바탕화면 바로가기에서 ClipDropper 실행

**대안: 설치 프로그램 없이 실행**

유일한 의존성은 무료 [.NET 8 SDK](https://dotnet.microsoft.com/download/dotnet/8.0)(~200 MB, 1회 설치)입니다.

1. [.NET 8 SDK](https://dotnet.microsoft.com/download/dotnet/8.0) 설치
2. 이 저장소 다운로드 또는 클론
3. `ClipDropper-Windows\run.bat` 더블클릭 — 자동으로 빌드하고 실행
