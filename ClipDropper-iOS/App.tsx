...
export default function App() {
  ...
  // Mirror of `status` readable from effects/callbacks with [] deps
  ...
  useEffect(() => {
    const sub = AppState.addEventListener('change', async (next) => {
      ...
      if (dev) {
        try {
          if (!(await dev.isConnected())) {
            // Link died while suspended — tear it down cleanly so
            // onDisconnected runs and the normal rescan path takes over
            dev.cancelConnection().catch(() => {});
            return;
          }
          const char = await dev.readCharacteristicForService(SERVICE_UUID, PC_TO_IOS_UUID);
          if (char.value) {
      ...
      } else if (!noAutoReconnect.current &&
                 (statusRef.current === 'idle' || statusRef.current === 'error' || statusRef.current === 'disconnected')) {
        // Connection was lost while suspended (timers/scans don't run in
        // background) — reconnect now that we're in the foreground
        scan();
      }
    });
    return () => sub.remove();
  ...
  async function scan() {
    if (statusRef.current === 'scanning') return; // a scan is already running
    noAutoReconnect.current = false;
    setStatus('scanning');
    setMsg('Scanning for ClipDropper PC…');
    // After backgrounding, iOS often keeps the BLE link half-alive ("zombie").
    // A connected peripheral never appears in scan results, so check for an
    // existing system-level connection first and reuse it directly.
    try {
      const known = await manager.connectedDevices([SERVICE_UUID]);
      if (known.length > 0) { connect(known[0]); return; }
    } catch { /* fall through to a normal scan */ }
    const t = setTimeout(() => {
  ...
  async function connect(device: Device) {
    try {
      ...
      // Clear any half-open link so future scans can find the PC again
  ...
  function handleError(e: BleError | Error) {
    ...
    if (code === BleErrorCode.BluetoothPoweredOff || code === BleErrorCode.BluetoothResetting) {
      setStatus('idle');
      setMsg('Bluetooth is off. Enable it, then tap Connect.');
      return;
    }
    setStatus('error');
